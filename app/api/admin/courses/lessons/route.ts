...
// POST create / PUT update / DELETE a course lesson.
export async function POST(request: Request) {
  const auth = await requireAdmin()
  if ("error" in auth) return NextResponse.json({ error: auth.error }, { status: auth.status })

  const { course_id, section_id, title, description, video_url, duration, attachment_url, display_order } =
    await request.json()
...
export async function PUT(request: Request) {
  ...
  const { data, error } = await supabaseAdmin.from("course_lessons").update(fields).eq("id", id).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  return NextResponse.json(data)
}
